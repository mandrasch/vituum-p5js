      ...
      for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
          ...
          if (hoveredCell !== null && hoveredCell.i === i && hoveredCell.j === j) {
            // Rotate the cell slightly
            let rotation = p.random(-p.PI / 16, p.PI / 16); // Rotate by a random angle between -PI/16 and PI/16
            p.push();
            p.translate(x + cellWidth / 2, y + cellHeight / 2);
            p.rotate(rotation);

            // Get the region of the image corresponding to this cell
            let cellImage = img.get(x, y, cellWidthWithPadding, cellHeightWithPadding);
            p.image(cellImage, -cellWidthWithPadding / 2, -cellHeightWithPadding / 2);

            p.pop(); // Reset transformation
          } else {
            // Draw the cell without rotation
            let cellImage = img.get(x, y, cellWidthWithPadding, cellHeightWithPadding);
            p.image(cellImage, x, y);
          }
        }
      }
    }
